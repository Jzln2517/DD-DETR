...
class rtdetr_heatmap:
    def __init__(self, weight, device, method, layer, backward_type, conf_threshold, ratio, show_box, renormalize):
        ...
        # 根据设备类型（CPU/GPU）设置 use_cuda 参数
        use_cuda = device.type == 'cuda'  # 如果是 GPU 设备则为 True，否则为 False
        # method = eval(method)(model=model, target_layers=target_layers, device=device)
        method = eval(method)(model=model, target_layers=target_layers, use_cuda=use_cuda)
...
def get_params():
    params = {
        'weight': r'E:\Code\RTDETR610\RTDETR-main\runs\train\rtdetr-C2f-IEL-HyperCompute-MFM-100\weights\best.pt',
        'device': 'cuda:0',
...
if __name__ == '__main__':
    ...
    model(r'D:\Datasets\VisDrone2019\images\val', 'Heatmap-100')